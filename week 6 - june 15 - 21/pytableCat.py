...
#PYTABLE + MULTIPROCESSING IMPLEMENTATION
...
t0 = time()
testH5file= tables.open_file('/disk1/bsesar/projects/PS1/proper_motion/prashansa/TriAnd_galaxies.h5')
t1= time()
print t1 - t0
...
#save unique galaxy ids in another variable -- note the brackets in 'tableName.col'
uniqueObjIDfile = np.unique(triand.col('obj_id'))

#the number of observations actually available for each object
noOfObs = np.zeros(len(uniqueObjIDfile)) 
  
#binning mjds for once
mjdSorted = np.sort(triand.col('mjd'))
