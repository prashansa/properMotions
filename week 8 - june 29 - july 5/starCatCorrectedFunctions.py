...
def calcMedianAndResiduals(table, debug):
    ...
    #for i in np.arange(30-1)+1:
        ...
        if (objID == currObjID):
            ...
            if(noOfObsArray[objIDcounter] >= 3.0):
    ...
    if debug: print 'processing the last object now'
    if(noOfObsArray[objIDcounter] >= 3.0):
    ...
    if (len(mjdBreakAt)>=3.0):
        ...
        for var1, var2 in enumerate(mjdBreakAt, start=0):
            ...
            if any(mjdIndex):
                ...
                dexInPixel[mjdIndexInPixel] -=  medianResidualDecEpochWise[var1+1]
            previousVar2 = var2
...
def pushDataInTable(ref):
    ...
    cursor = db.cursor()
    cursor.execute(''' CREATE TABLE finalData(objID INT NOT NULL, ra REAL NOT NULL, dec REAL NO NULL, raError REAL NOT NULL, decError REAL NOT NULL)''')

    db.commit()

    dat = starCatCorrectedFunctions.pixelTasks(parameterListForPixel[20])

    
    cursor.execute('INSERT INTO finalData (objID, ra, dec, raError, decError) VALUES (?, ?, ?, ?, ?)', dat)
