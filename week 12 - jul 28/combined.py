...
for chunkNo, value in enumerate(raMinArray):
    ...
    if (noOfRows!= countForRows):
        print "error : the no. of rows dont match for chunk no", chunkNo, "with bounds", raMin, decMin, raMax, decMax
        break
    t0 = time()
    testH5file = tables.open_file('/a41233d1/gupta/files/%s.h5' %h5fileName)
    # load the table
    ...
    print "MJD break points for this file", mjdBreakAt
    ...
    db = sqlite3.connect('/a41233d1/gupta/databases27July2015/mydb%d'%chunkNo)
